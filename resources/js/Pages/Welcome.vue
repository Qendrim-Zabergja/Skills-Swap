      ...
      <main>
        ...
        <section class="py-12 px-4 max-w-6xl mx-auto">
          <div class="text-center mb-10">
            <h1 class="text-3xl font-bold mb-2">Swap Skills, Learn & Teach For Free</h1>
            <p class="text-gray-600 mb-6">Exchange your expertise with others. No money involved - just knowledge sharing.</p>
            <Link :href="route('register')" class="bg-black text-white px-4 py-2 rounded">Get Started</Link>
          </div>
        </section>
        ...
        <section class="py-8 px-4 max-w-6xl mx-auto">
            ...
            <div v-for="skill in featuredSkills" :key="skill.id" class="border rounded-lg p-4">
              <div class="flex items-center mb-4">
                <div class="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center mr-3">
                  <img v-if="skill.user.profile_image" :src="'/storage/' + skill.user.profile_image" class="w-10 h-10 rounded-full object-cover" :alt="skill.user.first_name">
                  <span v-else class="text-gray-500">{{ getInitials(skill.user.first_name, skill.user.last_name) }}</span>
                </div>
                <div>
          ...
          <div v-else class="text-center py-10 bg-gray-50 rounded-lg">
            ...
            <Link v-if="$page.props.auth.user" :href="route('profile.edit')" class="bg-black text-white px-4 py-2 rounded mt-4 inline-block">Add Your Skills</Link>
            <Link v-else :href="route('register')" class="bg-black text-white px-4 py-2 rounded mt-4 inline-block">Sign Up to Share</Link>
          </div>
        </section>
        ...
        <section class="py-12 px-4 max-w-6xl mx-auto">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div class="text-center">
              ...
              <p class="text-sm text-gray-600">Browse skills you want to learn and connect with potential teachers.</p>
            </div>
            <div class="text-center">
              <div class="w-12 h-12 rounded-full bg-gray-200 flex items-center justify-center mx-auto mb-4">
              ...
              <p class="text-sm text-gray-600">Exchange your knowledge and learn new skills through teaching others.</p>
            </div>
          </div>
        </section>
      </main>
